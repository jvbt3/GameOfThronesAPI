import {Body, Injectable, UseInterceptors} from '@nestjs/common';
...
export class CharactersService {
  ...
  findOne(id: number) {
    return this.characterModel.findOne({id: id});
  }

  update(id: number, createCharacterDto: CreateCharacterDto) {
    return this.characterModel.updateOne({id: id}, createCharacterDto);
  }

  remove(id: number) {
    return this.characterModel.deleteOne({id: id});
  }
